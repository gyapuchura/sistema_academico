...
$sql_a =" SELECT idmacrocurricula, idobjetivo_anual, idarea_conocimiento, macrocurricula, idambito, idmodelo_educativo, codigo ";
$sql_a.=" FROM macrocurricula WHERE idmacrocurricula='$idmacrocurricula_ss' ";
...
$rowus = mysqli_fetch_array($resultus);?>
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-12">
				<h2 class="pageTitle">MACROCURRÍCULA  <?php echo $row_a[6]; ?></h2>
			</div>
		</div>
	</div>
	</section>
    ...
    <h4 align="center"><a href="macrocurriculas_l.php">VOLVER A MACROCURRÍCULAS</a></h4>
  </div>
</br>
...
<!-- javascript --->
<form name="FORM9" action="guarda_mod_macrocurricula_o.php" method="post">
<div class="box-area">
...
<?php
$sqlv = "SELECT idarea_conocimiento, area_conocimiento FROM area_conocimiento ";
...
<div class="row">
  ...
  <div class="col-md-8"><textarea class="form-control" rows="3" name="macrocurricula" required><?php echo $row_a[3]?> </textarea></div> 
</div>
...
<option selected>Seleccione</option>
<?php
$sqlv = "SELECT idambito, ambito FROM ambito ";
$resultv = mysqli_query($link,$sqlv);
if ($rowv = mysqli_fetch_array($resultv)){
mysqli_field_seek($resultv,0);
while ($fieldv = mysqli_fetch_field($resultv)){
...
?>
<option value="<?php echo $rowv[0];?>" <?php if ($rowv[0]==$row_a[4]) echo "selected";?> ><?php echo $rowv[1];?></option>
<?php
} while ($rowv = mysqli_fetch_array($resultv));
} else {
}
...
<option selected>Seleccione</option>
<?php
$sqlv = "SELECT idmodelo_educativo, modelo_educativo FROM modelo_educativo ";
$resultv = mysqli_query($link,$sqlv);
if ($rowv = mysqli_fetch_array($resultv)){
mysqli_field_seek($resultv,0);
while ($fieldv = mysqli_fetch_field($resultv)){
...
<div class="row">
  ...
  MODIFICAR DATOS DE LA MACROCURRICULA
  </button>
  </div>
  </div>
</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">REGISTRO CAMBIOS A LA MACROCURRICULA</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ¿Esta seguro de Modificar la nueva Macrocurricula?
       </div>
